...
public class Controller {
	...
	public void setClientRefreshRate() {
		try {			
			...
			if(newRefreshRate>=lowerBoundRR+0.1 && newRefreshRate<=upperBoundRR+0.1) {
				this.clientRefreshRate=(double)Math.round(newRefreshRate*10.0)/10.0;	//round to 1 decimal point
				if(this.clientRefreshRate<=this.subserverRefreshRate) {					//because subserverRR must be lower than clientRR
	...
	public void setSubserverRefreshRate() {
		try {
			...
			if(newRefreshRate>=lowerBoundRR && newRefreshRate<=upperBoundRR) {
				this.subserverRefreshRate=(double)Math.round(newRefreshRate*10.0)/10.0;		//round to 1 decimal point
				if(this.clientRefreshRate<=this.subserverRefreshRate) {					//because subserverRR must be lower than clientRR
	...
	public void stopServer() {
		...
		reqListener.stopListeningToRequests();
	}
